...
with tf.variable_scope('accuracy'):
    ...
    #accuracy = (accuracy * 1.0)/tf.size(y_[:,0])
    tf.summary.scalar('accuracy', accuracy)

session = tf.Session()
saver = tf.train.Saver()
save_path = os.path.join('checkpoints_6layers_copy8/', 'svhn_multi_v5_6layers')

try:
    print("Restoring last checkpoint ...")
    last_chk_path = tf.train.latest_checkpoint(checkpoint_dir='checkpoints_6layers_copy8')
    print(last_chk_path)
