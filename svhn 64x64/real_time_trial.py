...
resized_images = []
for i in range(1,42):
    img = cv2.imread('Real_Time_Images/image_'+str(i)+'.jpeg',0)
    images.append(img)
...
# cv2.imshow('Image_1',resized_images[0])
# cv2.waitKey(2000)
# cv2.imshow('Image_2',resized_images[1])
# cv2.waitKey(2000)
...
#save_path = os.path.join('checkpoints_6layers_copy2/', 'svhn_multi_v5_6layers')
...
#LOG_DIR = 'logs_6layers_copy2/'
#merged = tf.summary.merge_all()
#train_writer = tf.summary.FileWriter(LOG_DIR,graph=tf.get_default_graph())
# batch_size = 128 
d3 = 0.85

def evaluate_data(images):
    for img in images:
        #i = cv2.resize(img,(images.index(img),img.shape[0],img.shape[1],1))
        x_data = np.array(img).flatten().reshape(-1,64,64,1)
        output = session.run(y_pred_cls,{x: x_data, p_keep_3: 1.})
        #print("Output : {}".format(output))
        a = output[0]
        a = str(a)
        print(a)

evaluate_data(resized_images)
...
# def evaluate_data(test, batch_size):
