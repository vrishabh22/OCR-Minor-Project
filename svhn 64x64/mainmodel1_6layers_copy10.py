...
# image = tf.placeholder(tf.float32, shape=(None, 54, 54, num_channels), name='image')
# image = tf.random_crop(x, [54,54,3])
# x = tf.image.resize_image_with_crop_or_pad(x, 54, 54)
...
with tf.variable_scope("loss"):
    loss1 = tf.reduce_sum(tf.nn.sparse_softmax_cross_entropy_with_logits(logits=logits_1, labels=y_[:, 0]))
    loss2 = tf.reduce_sum(tf.nn.sparse_softmax_cross_entropy_with_logits(logits=logits_2, labels=y_[:, 1]))
    loss3 = tf.reduce_sum(tf.nn.sparse_softmax_cross_entropy_with_logits(logits=logits_3, labels=y_[:, 2]))
    loss4 = tf.reduce_sum(tf.nn.sparse_softmax_cross_entropy_with_logits(logits=logits_4, labels=y_[:, 3]))
    loss5 = tf.reduce_sum(tf.nn.sparse_softmax_cross_entropy_with_logits(logits=logits_5, labels=y_[:, 4]))
    loss = ((loss1 + loss2 + loss3 + loss4 + loss5)*1.0)/tf.cast(tf.size(y_[:,0]),tf.float32)
...
with tf.variable_scope('accuracy'):
    ...
    accuracy = tf.reduce_sum(correct_prediction) * 100.0
    accuracy = (accuracy * 1.0)/tf.cast(tf.size(y_[:,0]),tf.float32)
    tf.summary.scalar('accuracy', accuracy)

session = tf.Session()
saver = tf.train.Saver()
save_path = os.path.join('checkpoints_6layers_copy10/', 'svhn_multi_v5_6layers')

try:
    print("Restoring last checkpoint ...")
    last_chk_path = tf.train.latest_checkpoint(checkpoint_dir='checkpoints_6layers_copy10')
    print(last_chk_path)
...
# for i in range(10):
    # print(i)
optimize(num_iterations=10000, display_step=200)
